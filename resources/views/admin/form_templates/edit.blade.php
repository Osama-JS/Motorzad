...
@section('title', __('تعديل قالب النموذج'))
...
@section('content')
<x-admin-header :title="__('تعديل قالب التسجيل')" :breadcrumb="__('تعديل قالب')">
...
{{-- Alpine Component --}}
<div x-data="formBuilder(@js($formTemplate->fields))" class="row justify-content-center">
    <div class="col-lg-8">
        ...
        <form action="{{ route('admin.form-templates.update', $formTemplate->id) }}" method="POST" id="templateForm">
            @csrf
            @method('PUT')
                ...
                <div class="card-body p-4">
                    <div class="row g-4">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="name" x-model="templateName" class="form-control" placeholder="{{ __('مثال: نموذج تسجيل المعارض 2026') }}" required value="{{ old('name', $formTemplate->name) }}">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="description" x-model="templateDescription" class="form-control" placeholder="{{ __('وصف داخلي للهدف من هذا القالب') }}" value="{{ old('description', $formTemplate->description) }}">
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body p-4">
                    ...
                    <div class="d-flex flex-column gap-4 mb-4" x-ref="sortableContainer">
                                            ...
                                            <div class="row g-3">
                                                ...
                                                <div class="col-12" x-show="['select', 'radio', 'multi-select'].includes(field.type)" x-transition>
                                                    <div class="bg-danger bg-opacity-10 p-3 rounded-3 border border-danger border-opacity-25 mt-2">
                                                        ...
                                                        <input type="text" x-model="field.options_string" :name="'fields['+index+'][options]'" class="form-control shadow-sm" placeholder="خيارات مفصولة بفاصلة (مثال: أزرق, أحمر, أخضر)">
                        ...
                        <div x-show="fields.length === 0" class="text-center py-5 bg-body border border-dashed rounded-4 shadow-sm" style="border-width: 2px !important; border-color: #cbd5e1 !important;">
                            ...
                            <h5 class="text-dark fw-bold">{{ __('تم حذف جميع الحقول') }}</h5>
                            <p class="text-muted mb-0">{{ __('يجب عليك إضافة حقل واحد على الأقل قبل حفظ القالب.') }}</p>
                        </div>
                    </div>
            ...
            <div class="d-flex justify-content-end gap-2 mb-5 mt-4">
                <button type="submit" class="btn btn-save-template btn-lg rounded-pill px-5 py-3 fw-bold">
                    <i class="fa-solid fa-cloud-arrow-up me-2 fs-5 align-middle"></i> <span class="align-middle">{{ __('حفظ وتحديث القالب') }}</span>
                </button>
            </div>
        </form>
    </div>
                                ...
                                <div>
                                    ...
                                    <template x-if="field.type === 'select'">
                                        <select class="form-select bg-light" x-model="previewData[field.name]">
                                            ...
                                            <template x-for="option in (field.options_string || '').split(',').filter(o => o.trim() !== '')">
                                    ...
                                    <template x-if="field.type === 'radio'">
                                        <div>
                                            <template x-for="(option, idx) in (field.options_string || '').split(',').filter(o => o.trim() !== '')">
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('formBuilder', (initialFields = []) => ({
            fields: [],
            fieldIdCounter: 1,
            templateName: '{{ old('name', $formTemplate->name) }}',
            templateDescription: '{{ old('description', $formTemplate->description) }}',
            previewData: {},
            ...
            init() {
                if (initialFields && initialFields.length > 0) {
                    this.fields = initialFields.map(f => ({
                        ...
                        has_condition: f.depends_on_field_name ? true : false,
                        depends_on_field_name: f.depends_on_field_name || '',
            ...
            removeField(index) {
                if(confirm('هل أنت متأكد من حذف هذا الحقل؟ قد يؤدي هذا لفقدان بيانات البائعين السابقة المرتبطة بهذا الحقل.')) {
