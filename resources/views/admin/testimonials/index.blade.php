...
<div class="page-header">
    <div>
        <h1>{{ __('Testimonials Management') }}</h1>
        <div class="breadcrumb"><a href="{{ route('admin.dashboard') }}">{{ __('Dashboard') }}</a> / {{ __('Testimonials') }}</div>
    </div>
    ...
    <div class="col-12 col-sm-6 col-md-4">
        <div class="stat-card blue h-100 stat-card-compact shadow-sm border-0">
            ...
            <div>
                ...
                <div class="stat-label" style="font-size: 0.75rem !important;">{{ __('Total Testimonials') }}</div>
            </div>
        </div>
    </div>
    <!-- Active Testimonials -->
    <div class="col-12 col-sm-6 col-md-4">
        <div class="stat-card green h-100 stat-card-compact shadow-sm border-0">
            ...
            <div>
                ...
                <div class="stat-label" style="font-size: 0.75rem !important;">{{ __('Active Testimonials') }}</div>
            </div>
        </div>
    </div>
    ...
    <div class="col-12 col-sm-6 col-md-4">
        <div class="stat-card red h-100 stat-card-compact shadow-sm border-0">
            ...
            <div>
                ...
                <div class="stat-label" style="font-size: 0.75rem !important;">{{ __('Inactive Testimonials') }}</div>
            </div>
        </div>
    </div>
</div>
        ...
        <div class="row g-3 align-items-center">
            <div class="col-md-5">
                <div class="input-group">
                    ...
                    <input type="text" id="filter_search" class="form-control border-start-0 ps-0" placeholder="{{ __('Search Testimonials...') }}">
                </div>
            </div>
            <div class="col-md-4">
            ...
            <div class="col-md-3 text-end">
                <button type="button" class="btn btn-secondary w-100" onclick="fetchTestimonials(1)">
                    {{ __('Filter') }}
                </button>
            </div>
        </div>
...
<div class="view-toolbar mb-3 d-flex justify-content-between align-items-center">
    <div class="d-flex align-items-center gap-3">
        <div class="d-flex align-items-center">
            ...
            <select id="filter_per_page" class="form-select form-select-sm select2-init" style="width: 80px;" onchange="fetchTestimonials(1)">
        ...
        <div class="dropdown">
            ...
            <div class="dropdown-menu shadow-sm p-3" style="min-width: 200px;">
                ...
                <div class="form-check mb-2">
                    <input class="form-check-input col-toggle" type="checkbox" id="col_name" value="1" checked disabled>
                    <label class="form-check-label" for="col_name">{{ __('Name & Role') }}</label>
                </div>
                <div class="form-check mb-2">
...
<div id="table-view-container" class="card shadow-sm border-0 mb-4">
    <div class="table-responsive">
        <table class="table table-hover table-striped align-middle mb-0 w-100" id="testimonials-custom-table">
            <thead class="table-light">
                <tr>
                    <th class="border-bottom-0 col-toggle-0">#</th>
                    <th class="border-bottom-0 col-toggle-1">{{ __('Name & Role') }}</th>
            ...
            <tbody id="custom-testimonials-tbody">
        ...
        <div class="modal-content border-0 shadow-lg" style="border-radius: var(--radius-lg); background: var(--bg-card); color: var(--text-color);">
            <form id="testimonialForm">
                ...
                <div class="modal-header border-bottom-0 pb-0">
                    <h5 class="modal-title fw-bold fs-4 d-flex align-items-center gap-2" id="testimonialModalLabel">
                        ...
                        <span id="modalTitleText">{{ __('Add New Testimonial') }}</span>
                    </h5>
                            ...
                            <div class="card h-100 border-0 shadow-none" style="background: var(--bg-body); border-radius: var(--radius-lg);">
                                <div class="card-body p-4">
                                    ...
                                    <div class="form-group mb-4">
                                        <label for="name_ar" class="form-label fw-bold d-flex justify-content-between align-items-center w-100">
                                            <span>{{ __('Name') }} <span class="text-danger">*</span></span>
                                            <x-translate-button from="#name_ar" to="#name_en" />
                                        </label>
                                        <div class="position-relative">
                                            <input type="text" class="form-control form-control-lg px-3 py-2" id="name_ar" name="name_ar" placeholder="أدخل الاسم بالعربية..." required style="background: var(--bg-input); color: var(--text-color); border: 1px solid var(--border);">
                                        </div>
                                    </div>
                                    
                                    <div class="form-group mb-4">
                                        <label for="role_ar" class="form-label fw-bold d-flex justify-content-between align-items-center w-100">
                                            <span>{{ __('Role') }} <span class="text-danger">*</span></span>
                                            <x-translate-button from="#role_ar" to="#role_en" />
                                        </label>
                                        <div class="position-relative">
                                            <input type="text" class="form-control form-control-lg px-3 py-2" id="role_ar" name="role_ar" placeholder="مثال: عميل جديد..." required style="background: var(--bg-input); color: var(--text-color); border: 1px solid var(--border);">
                                        </div>
                                    </div>

                                    <div class="form-group mb-4">
                                        <label for="text_ar" class="form-label fw-bold d-flex justify-content-between align-items-center w-100">
                                            <span>{{ __('Review Text') }} <span class="text-danger">*</span></span>
                                            <x-translate-button from="#text_ar" to="#text_en" />
                                        </label>
                                        <textarea class="form-control px-3 py-2" id="text_ar" name="text_ar" rows="4" placeholder="أدخل الرأي بالعربية..." required style="background: var(--bg-input); color: var(--text-color); border: 1px solid var(--border);"></textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="col-lg-6">
                            <div class="card h-100 border-0 shadow-none" style="background: var(--bg-body); border-radius: var(--radius-lg);">
                                <div class="card-body p-4">
                                    ...
                                    <div class="form-group mb-4">
                                        <label for="name_en" class="form-label fw-bold">{{ __('Name') }} <span class="text-danger">*</span></label>
                                        <div class="position-relative">
                                            <input type="text" class="form-control form-control-lg px-3 py-2" id="name_en" name="name_en" placeholder="Enter name in English..." required style="background: var(--bg-input); color: var(--text-color); border: 1px solid var(--border);">
                                        </div>
                                    </div>
                                    
                                    <div class="form-group mb-4">
                                        <label for="role_en" class="form-label fw-bold">{{ __('Role') }} <span class="text-danger">*</span></label>
                                        <div class="position-relative">
                                            <input type="text" class="form-control form-control-lg px-3 py-2" id="role_en" name="role_en" placeholder="E.g. New Customer..." required style="background: var(--bg-input); color: var(--text-color); border: 1px solid var(--border);">
                                        </div>
                                    </div>

                                    <div class="form-group mb-4">
                                        <label for="text_en" class="form-label fw-bold">{{ __('Review Text') }} <span class="text-danger">*</span></label>
                                        <textarea class="form-control px-3 py-2" id="text_en" name="text_en" rows="4" placeholder="Enter review in English..." required style="background: var(--bg-input); color: var(--text-color); border: 1px solid var(--border);"></textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="row mt-4">
                        <div class="col-md-6">
                            <div class="form-group mb-0">
                                <label for="avatar_init" class="form-label fw-bold">{{ __('Avatar Initial (Arabic)') }}</label>
                                <input type="text" class="form-control" id="avatar_init" name="avatar_init" placeholder="مثال: أ" maxlength="2" style="background: var(--bg-input); color: var(--text-color); border: 1px solid var(--border);">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group mb-0">
                                <label for="avatar_init_en" class="form-label fw-bold">{{ __('Avatar Initial (English)') }}</label>
                                <input type="text" class="form-control" id="avatar_init_en" name="avatar_init_en" placeholder="e.g. A" maxlength="2" style="background: var(--bg-input); color: var(--text-color); border: 1px solid var(--border);">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-4 pt-4 border-top" style="border-color: var(--border-light) !important;">
                        <label class="checkbox-item w-100 mb-0 d-flex align-items-center p-3 rounded" style="background: var(--bg-body);">
                            ...
                            <div class="ms-3">
                                ...
                                <div class="check-sub text-muted mt-1">{{ __('Make this testimonial visible to users immediately.') }}</div>
                            </div>
                        </label>
                    </div>
...
<script>
    window.TestimonialConfig = {
        csrf: '{{ csrf_token() }}',
        urls: {
            data: "{{ route('admin.testimonials.data') }}",
            store: "{{ route('admin.testimonials.store') }}",
            update: "{{ route('admin.testimonials.update', ':id') }}",
            destroy: "{{ route('admin.testimonials.destroy', ':id') }}",
            toggleActive: "{{ route('admin.testimonials.toggle-active', ':id') }}"
        },
        trans: {
            loading: "{{ __('Loading...') }}",
            ...
            deleteTestimonial: "{{ __('Delete Testimonial?') }}",
