$(document).ready(function() {
    $.ajaxSetup({
        headers: {
            'X-CSRF-TOKEN': window.WalletConfig.csrf
        }
    });
    ...
    window.fetchWallets = function(page = 1) {
        ...
        $.ajax({
            url: window.WalletConfig.urls.data,
            data: {
                page: page,
                search: search,
                per_page: perPage
        ...
        data.forEach(wallet => {
            let tr = `
                <tr>
                    ...
                    <td>${wallet.debt_ceiling}</td>
                    <td>${wallet.debt_usage}</td>
                    <td>${wallet.total_deposits}</td>
                    <td>${wallet.total_withdrawals}</td>
    ...
    $('#debtForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            error: function(xhr) {
                Swal.fire(window.WalletConfig.trans.error, window.WalletConfig.trans.operationFailed, 'error');
            }
        });
    });
});
